...
select JOB_ID,avg(salary),avg(salary),min(salary),max(salary),sum(salary),count(*) from EMPLOYEES
group by JOB_ID;
...
--display how many depsrtments we have in each location
